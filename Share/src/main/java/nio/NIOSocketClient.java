			...
			while(!done){
				...
				while(iterator.hasNext()){
					...
					if(key.isConnectable() && !socketChannel.isConnected()){
						...
						boolean success = socketChannel.connect(new InetSocketAddress(addr, 1111));
					...
					if(key.isWritable() && !written){
						//向套接字通道中写入数据
						if(i < 10) socketChannel.write(ByteBuffer.wrap(new String("howdy"+ i +'\n').getBytes()));
						else if(i == 10)socketChannel.write(ByteBuffer.wrap(new String("END").
								getBytes()));
						written = true;
						i++;
					}
